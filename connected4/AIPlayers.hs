...
instance Player NaiveAIPlayer where
  nextMove brd _ = return $ bestMove positionalHeuristic nextPossibleBoards maxBound brd 4

data DecentAIPlayer = DecentAIPlayer

instance Player DecentAIPlayer where
  nextMove brd _ = return $ bestMove (smartHeuristic $ allCollinear $ config brd) nextPossibleBoards maxBound brd 8
